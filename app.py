...
# –––––––– إرسال ذكر/دعاء تلقائي (مع احترام الإيقاف) ––––––––
...
```
# إرسال للمستخدمين (فقط من لم يوقف التذكير)
for uid in target_users:
    if uid not in notifications_off:  # ✅ التحقق من الإيقاف
        try:
            line_bot_api.push_message(uid, TextSendMessage(text=message))
        except:
            pass

# إرسال للمجموعات (فقط من لم توقف التذكير)
for gid in target_groups:
    if gid not in notifications_off:  # ✅ التحقق من الإيقاف
        try:
...
```
    ...
    # ---------------- أمر ذكرني (إرسال للمرسل فقط) ---------------- #
    if user_text.lower() == "ذكرني":
        category = random.choice(["duas", "adhkar", "hadiths", "quran"])
        message = random.choice(content.get(category, ["لا يوجد محتوى"]))

        # ✅ الرد فقط للمستخدم أو المجموعة التي أرسلت الأمر
        try:
