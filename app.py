...
def send_random_message():
    category = random.choice(["duas", "hadiths", "adhkar"])
...
def message_loop():
    while True:
        send_random_message()
        time.sleep(random.randint(3600, 5400))  # بين ساعة و 1.5 ساعة
...
def handle_message(event):
    ...
    # إرسال رسالة أولية عند أول تواصل
...
1. ذكرني
   - يرسل دعاء أو حديث أو ذكر عشوائي للجميع.

2. تسبيح
   - عرض عدد التسبيحات الخاصة بك.
...
"""
    ...
    if user_text.lower() == "تسبيح":
        ...
        status = f"سبحان الله: {counts['سبحان الله']}/{tasbih_limits}\nالحمد لله: {counts['الحمد لله']}/{tasbih_limits}\nالله أكبر: {counts['الله أكبر']}/{tasbih_limits}"
