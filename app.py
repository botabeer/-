...
# تحميل محتوى الأذكار والفضل
content = load_json(os.path.join(CONTENT_DIR, "content.json"), {"duas": [], "adhkar": [], "hadiths": [], "quran": []})
fadl_content = load_json(os.path.join(CONTENT_DIR, "fadl.json"), {"fadl": []}).get("fadl", [])
morning_adhkar = load_json(os.path.join(CONTENT_DIR, "morning_adhkar.json"), {"adhkar": []}).get("adhkar", [])
